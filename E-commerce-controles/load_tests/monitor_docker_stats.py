...
def main() -> None:
    ...
    # Almacenamiento de picos (máximos) y acumuladores para promedios
    peaks: dict[str, dict[str, Any]] = defaultdict(
        lambda: {"cpu_peak": 0.0, "mem_peak_mib": 0.0, "mem_raw_peak": "", "mem_perc_peak": 0.0,
                 "cpu_sum": 0.0, "cpu_samples": 0, "mem_sum_mib": 0.0, "mem_samples": 0}
    )
    ...
    try:
        while not os.path.exists(stop_file):
            ...
            for line in lines:
                ...
                # Acumular para promedios (ecuacion CPU_% = (1/k) sum(Uj) de la tesis)
                peaks[name]["cpu_sum"] += cpu_val
                peaks[name]["cpu_samples"] += 1
                peaks[name]["mem_sum_mib"] += mem_val
                peaks[name]["mem_samples"] += 1
    ...
    # Tabla adicional de PROMEDIOS (metricas ISO/IEC 25023: CPU_% = (1/k) sum(Uj))
    report_lines.append("")
    report_lines.append("## PROMEDIOS DE CONSUMO (ventana 60s, ecuaciones 2 y 4 de la tesis)")
    report_lines.append("| Contenedor | CPU Promedio (%) | Memoria Promedio (MiB) | Muestras |")
    report_lines.append("| :--- | :---: | :---: | :---: |")
    for name, stats in sorted(peaks.items()):
        cpu_mean = stats["cpu_sum"] / stats["cpu_samples"] if stats["cpu_samples"] > 0 else 0.0
        mem_mean = stats["mem_sum_mib"] / stats["mem_samples"] if stats["mem_samples"] > 0 else 0.0
        report_lines.append(
            f"| **{name}** | {cpu_mean:.2f}% | ({mem_mean:.2f} MiB) | {stats['cpu_samples']} |"
        )
