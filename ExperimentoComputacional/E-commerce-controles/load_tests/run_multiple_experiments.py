...
def run_single_experiment(
    ...
    try:
        ...
        try:
            ...
            print("[WARN] k6 colgó. Abortando iteración actual.")
...
def main() -> None:
    ...
    for i in range(1, args.runs + 1):
        ...
        if i < args.runs:
            ...
            subprocess.run(["docker-compose", "down"], cwd=env_dir, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=60)
            for _retry in range(3):
                ...
                time.sleep(3)
                result = subprocess.run(["docker-compose", "up", "-d"], cwd=env_dir, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE, timeout=120)
                if result.returncode == 0:
                    break
                subprocess.run(["docker-compose", "down", "--remove-orphans"], cwd=env_dir, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=60)
                time.sleep(3)
            else:
                print(f"  [ERROR] No se pudo levantar el entorno en {env_dir} despues de 3 intentos. Abortando.")
                sys.exit(1)
            time.sleep(5)
