...
UC_LEVELS = {
    ...
    "UC-03": {"max": 5, "label": "Ciclos (1-5)",      "scale": list(range(1,6))},
    "UC-04": {"max": 9, "label": "Aliases (Fibonacci)","scale": [1,2,3,5,8,13,21,34,55]},
    "UC-05": {"max": 5, "label": "Factor (1-5)",       "scale": list(range(1,6))},
}
...
def parse_md_full(md_text):
    ...
    # --- Datos por run de metricas K6 ---
    run_metrics = {}
    for line in txt.splitlines():
        m = re.match(r'\|\s*Run\s*(\d+)\s*\|\s*([\d.]+)\s*\|\s*([\d.]+)\s*ms\s*\|\s*([\d.]+)%', line)
        if m:
            n = int(m.group(1))
            run_metrics[n] = {
    ...
    for line in txt.splitlines():
        for cont in containers:
            if cont in line and "|" in line:
                ...
                for p in parts:
                    mm = re.search(r'([\d.]+)%', p)
                    if mm: pcts.append(float(mm.group(1)))
                if pcts:
                    cpu[cont] = {i+1: v for i,v in enumerate(pcts[:3])}
                break
    ...
    for line in txt.splitlines():
        for cont in containers:
            if cont in line and "MiB" in line:
                ...
                if mibs:
                    ram[cont] = {i+1: v for i,v in enumerate(mibs[:3])}
                break

    # Promedios generales
    avg = {"peticiones":0.0,"latencia":0.0,"fallos":0.0,"cpu_gw":0.0,"cpu_top":0.0}
    txt_clean = txt.replace('*', '') # Eliminar asteriscos para evitar catastrophic backtracking en regex
