...
def parse_md_report(md_path):
    ...
    for c in CONTAINERS:
        ...
        if m:
            ...
            if len(vals) >= 3:
                cpu[c] = {i+1: float(vals[i]) for i in range(3)}
    
    # RAM por contenedor y run
    ram = {}
    in_ram = False
        ...
        if "PICOS DE MEMORIA" in line:
            ...
            for c in CONTAINERS:
                if f"**{c}**" in line:
                    ...
                    if mibs:
                        ram[c] = {i+1: float(mibs[i]) for i in range(min(3, len(mibs)))}
...
def export_uc01_excel(full_data, avg_data, vus_completed, out_path, breaking_point=None):
    ...
    HEADERS = [
        ...
        "Fecha", "Hora", "Throughput (req/s)", "Latencia (ms)", "Fallos (%)",
        "GW CPU (%)", "ms-usuarios CPU (%)", "ms-catalogo CPU (%)",
        "ms-resenas CPU (%)", "ms-ordenes CPU (%)", "Mongo CPU (%)",
        "Postgres CPU (%)", "GW RAM (MiB)", "ms-usuarios RAM (MiB)",
        "ms-catalogo RAM (MiB)", "ms-resenas RAM (MiB)", "ms-ordenes RAM (MiB)",
        "Mongo RAM (MiB)", "Postgres RAM (MiB)", "Tipo"
    ]
    ...
    for vus in vus_completed:
        for env_name in ["Vulnerable", "Protegido"]:
            ...
            if not runs:
                ...
                cell.fill = fill; cell.alignment = center; cell.border = bdr
                cell = ws.cell(row=row_idx, column=25, value="Sin Datos")
                cell.fill = fill; cell.alignment = center; cell.border = bdr
                row_idx += 1
                continue
            ...
            for run in runs:
                ...
                vals = [
                    ...
                    run['fecha'], run['hora'],
                    round(run['peticiones'], 2), round(run['latencia'], 2),
                    round(run['fallos'], 2), round(run.get('cpu_gw', 0), 2),
...
def generate_uc01_graphs(avg_data, vus_completed, breaking_point=None):
    ...
    BG = "#0d1117"; BG2 = "#161b22"; BORD = "#30363d"
    ...
    plt.rcParams.update({
        ...
        "grid.linestyle": "--", "legend.facecolor": "#21262d",
    ...
    for vus in FIBONACCI_VUS:
        ...
        for env_name, env_path in ENVS.items():
            ...
            try:
                ...
                if p.returncode != 0:
                    print(f"    [ERROR] El orquestador de K6 falló con código {p.returncode}. Abortando.")
                    exit(1)
            except subprocess.TimeoutExpired:
